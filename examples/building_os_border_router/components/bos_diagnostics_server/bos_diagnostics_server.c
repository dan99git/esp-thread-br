...
static esp_err_t device_id_reset_post_handler(httpd_req_t *req)
{
    if (!bos_commissioning_request_authorized(req)) {
        return send_unauthorized(req);
    }
...
static bool trigger_resolve_eui64(otInstance *instance,
                                  const char *eui64,
                                  proxy_target_t *out,
                                  proxy_addr_scope_t *scope_out)
{
...
static esp_err_t phonebook_parse_csv(const char *raw,
                                     bos_phonebook_t *out,
...
static esp_err_t phonebook_validate_push_headers(httpd_req_t *req,
...
static esp_err_t phonebook_floating_get_handler(httpd_req_t *req)
{
...
static esp_err_t phonebook_post_handler(httpd_req_t *req)
{
    if (!bos_commissioning_request_authorized(req)) {
        return send_unauthorized(req);
    }
...
static bool phonebook_resolve_eui64(otInstance *instance,
                                    const char *eui64,
                                    proxy_target_t *out,
                                    bool *matched_out,
                                    proxy_addr_scope_t *scope_out)
...
static bool phonebook_resolve_spatial_to_eui64(const char *spatial_id, char out_eui64[17])
{
...
static esp_err_t spatial_proxy_handler(httpd_req_t *req)
{
